...
def extract_numbers(text):
    ...
    nums = re.split(r'[^0-9,.]+', text)

    return [float(num.replace(',', '')) for num in nums if num and re.match(r'^\d', num)]
...
def latin_ish_words(text):
    ...
    fragments = {'tion', 'ex', 'ph', 'ost', 'ist', 'ast'}

    words = re.split(r'\W+', text)

    return [word for word in words if any(x in word for x in fragments)]
